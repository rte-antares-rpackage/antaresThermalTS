...
read_cluster_desc <- function(path) {
  ...
  infos <- read_info(path)



  setnames(infos, "pmin", "pmin_mw")
  setnames(infos, "for", "part_francaise")
  setnames(infos, "pmax", "pcn_mw")
  setnames(infos, "code_gp", "nom")
  setnames(infos, "groupe", "corresp_groupes")
  setnames(infos, "sec", "rp_rsfp_mw")
  setnames(infos, "commentaire", "commentaires")
  setnames(infos, "...7", "nodata")

  infos <- merge(x = infos, y = code_palier_dic, all.x = TRUE, by = "type_groupe")
